...
const APIS = new Map([
    ...
    [
        ...
        "https://sessionserver.mojang.com/session/minecraft/profile/b876ec32e396476ba1158438d83c67d4",
    ],
    [
        "Minecraft Web Services",
        "https://api.mojang.com/users/profiles/minecraft/technoblade",
    ],
]);
    ...
    run: async (input: input2) => {
        return new Promise(async (resolve, reject) => {
            let embed = new Discord.MessageEmbed().setTitle(
                "Mojang/Minecraft API Status"
            );
            for (const [k, v] of APIS) {
                await timeout(800, fetch(v))
                    .then((data) => {
