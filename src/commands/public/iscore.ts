...
module.exports = {
    run: async (input: input2) => {
        return new Promise(async (resolve, reject) => {
            ...
            const svg2 = '" xmlns="http://www.w3.org/2000/svg">\
                <defs>\
                    <style>\
                        @font-face {\
                            font-family: minecraft;\
                            src: url(\'' + base_path + '/minecraft.ttf\');\
                        }\
                    </style>\
            </defs>\
                <style>\
            ...
            await nbt.parse(data, async (e: any, d: any) => {
                ...
                let svg = svg1 + 300 + ' ' + (25 + 20 * scoreboard.length) + svg2 + input.args![0] + svg3;
                let position = 0;
                for (const [k, v] of scoreboard) {
                    if (k == "Total") {
                        svg += '<text style="text-anchor: start; font-size: 20px; fill: #FFFFFF;" x="3" y="' + (40 + 20 * position) + '">' + k + '</text>';
                    } else {
                        svg += '<text class="ign" x="3" y="' + (40 + 20 * position) + '">' + k + '</text>';
                    }
                    svg += '<text class="score" x="100%" y="' + (40 + 20 * position) + '">' + v + '</text>';
                    position += 1;
                }
                svg += "</svg>";
                await fs.writeFile(base_path + "/board.svg", svg, async () => {
                    await sharp(base_path + "/board.svg")
                        ...
                        .then(() => {
                            input.channel!.send({ files: [base_path + "/board.png"] });
                        });
                });
