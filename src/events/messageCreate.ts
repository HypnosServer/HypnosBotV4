...
module.exports.run = (client: client) => {
    client.on("messageCreate", msg => {
        ...
        if (client.commands!.get(command)!.help.adminOnly && !msg.member!.roles.cache.has(client.config.adminRole)){
            msg.channel.send("You aren't an admin")
            return
        }
        if(client.commands!.get(command)!.help.memberOnly && !msg.member!.roles.cache.has(client.config.memberRole)) {
            msg.channel.send("You aren't a member")
            return
        }
        if(client.commands!.get(command)!.help.staffOnly && !client.config.staff.includes(msg.author.id)) {
            msg.channel.send("This command is only for bot staff :|")
            return
        } 
        // runs the command
        client.commands!.get(command)!.run({msg: msg, args: args, client: client})
    })
}