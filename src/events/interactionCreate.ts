...
module.exports.run = (client: client) => {
    client.on("interactionCreate", (interaction) => {
        ...
        if (interaction.isCommand()) {
            ...
            if (client.commands!.get(commandName)!.help.adminOnly && !interaction.guild.members.cache.get(interaction.user.id)?.roles.cache.has(client.config.adminRole)){
                interaction.reply("You aren't an admin")
                return
            }
            if(client.commands!.get(commandName)!.help.memberOnly && !interaction.guild.members.cache.get(interaction.user.id)?.roles.cache.has(client.config.memberRole)) {
